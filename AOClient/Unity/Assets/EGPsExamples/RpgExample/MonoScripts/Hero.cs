...
public sealed class Hero : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        CombatEntity.Rotation = transform.GetChild(0).localRotation.eulerAngles;
    ...
    private void OnPreSpell(Entity combatAction)
    {
        ...
        {
            var forward = spellAction.InputTarget.Position - (float3)CombatEntity.ModelTrans.position;
            CombatEntity.ModelTrans.localRotation = Quaternion.LookRotation(forward);
        }
        else
        {
            CombatEntity.ModelTrans.localRotation = Quaternion.LookRotation((Vector3)spellAction.InputPoint - CombatEntity.ModelTrans.position);
        }
        DisableMove();
    ...
    public void Attack()
    {
        ...
        if (CombatEntity.AttackSpellAbility.TryMakeAction(out var action))
        {
            ...
            CombatEntity.GetComponent<AttributeComponent>().Attack.SetBase(ET.RandomGenerator.RandomNumber(600, 999));
    ...
    public async ETTask PlayThenIdleAsync(AnimationClip animation)
    {
        ...
        await ET.TimerComponent.Instance.WaitAsync((int)(animation.length * 1000), token);
        //if (isTimeout)
        {
            token = null;
            AnimationComponent.PlayFade(AnimationComponent.IdleAnimation);
