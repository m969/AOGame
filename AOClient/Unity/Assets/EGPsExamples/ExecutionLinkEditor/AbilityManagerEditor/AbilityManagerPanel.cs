...
namespace EGamePlay
{
    public class AbilityManagerPanel : MonoBehaviour
    {
        ...
        public void RefreshList()
        {
            ...
            var allconfigs = AO.CfgTables.Tables.TbSkills.DataList;
            ...
            foreach (var item in allconfigs)
            {
                ...
                btn.transform.Find("AbilityNameText").GetComponent<Text>().text = $"{item.Id} {item.Name}";
                btn.transform.Find("AbilityDesText").GetComponent<Text>().text = item.Description;
                btn.GetComponent<Button>().onClick.AddListener(() => {
                    var folder = AbilityManagerObject.Instance.SkillAssetFolder;
                    var obj = AssetDatabase.LoadAssetAtPath<SkillConfigObject>(folder + $"/Skill_{item.Id}.asset");
