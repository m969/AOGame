...
namespace EGamePlay.Combat
{
    ...
    public partial class ExecutionEffect : Entity
    {
        ...
        public override void Awake(object initData)
        {
            ...
            if (clipType == ExecuteClipType.ActionEvent)
            {
                Add<ExecutionTimeTriggerComponent>().StartTime = (float)ExecutionEffectConfig.StartTime;
            }
            else if (ExecutionEffectConfig.Duration > 0)
            {
                Add<ExecutionTimeTriggerComponent>().StartTime = (float)ExecutionEffectConfig.StartTime;
                Get<ExecutionTimeTriggerComponent>().EndTime = (float)ExecutionEffectConfig.EndTime;
            }
