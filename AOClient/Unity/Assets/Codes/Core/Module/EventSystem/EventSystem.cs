...
namespace ET
{
    public class EventSystem: Singleton<EventSystem>, ISingletonUpdate, ISingletonLateUpdate
    {
        ...
        public readonly Dictionary<Type, List<EventInfo>> allEvents = new();
        ...
        public void Add(Dictionary<string, Type> addTypes)
        {
            ...
            foreach (Type type in types[typeof (EventAttribute)])
            {
                ...
                foreach (object attr in attrs)
                {
                    ...
                    //Log.Debug($"{eventType.Name} {obj.GetType().Name}");
                    //if (eventType.Name == "EntryEvent2")
                    //{
                    //    var ms = obj.GetType().GetMethods();
                    //    //foreach (var item in ms)
                    //    //{
                    //    //    Log.Debug(item.Name);
                    //    //}
                    //    obj.GetType().GetMethod("Run").Invoke(obj, new object[] { });
                    //}
                }
            }
        ...
        public async ETTask PublishAsync<T>(Entity source, T a) where T : struct
        {
            ...
            //var t = typeof(T);
            //Log.Debug(t.FullName);
            //foreach (var item in this.allEvents.Keys)
            //{
            //    Log.Debug($"PublishAsync {item.FullName} == {t.FullName} {t == item}");
            //}
            if (!this.allEvents.TryGetValue(typeof(T), out iEvents))
            {
                //Log.Error($"event not found: {typeof(T).Name}");
                return;
            }
